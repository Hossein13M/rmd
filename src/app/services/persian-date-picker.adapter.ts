...
export const PERSIAN_DATE_FORMATS = {
    ...
    display: { dateInput: 'jYYYY/jMM/jDD', monthYearLabel: 'jYYYY jMMMM', dateA11yLabel: 'jYYYY/jMM/jDD', monthYearA11yLabel: 'jYYYY jMMMM' },
};
export class MaterialPersianDateAdapter extends DateAdapter<jalaliMoment.Moment> {
    ...
    getDateNames(): Array<string> {
        ...
        for (let i = 0; i < 31; i++) valuesArray[i] = String(i + 1);
        return valuesArray;
    }
    ...
    createDate(year: number, month: number, date: number): jalaliMoment.Moment {
        ...
        if (this.getMonth(result) !== month) throw Error(`Invalid date ${date} for month with index ${month}.`);
    ...
    parse(value: any, parseFormat: string | Array<string>): jalaliMoment.Moment | null {
        if (value && typeof value === 'string') return jalaliMoment(value, parseFormat, 'fa');
        return value ? jalaliMoment(value).locale('fa') : null;
    }

    format(date: jalaliMoment.Moment, displayFormat: string): string {
        date = this.clone(date);
        if (!this.isValid(date)) throw Error('JalaliMomentDateAdapter: Cannot format invalid date.');
    ...
    override deserialize(value: any): jalaliMoment.Moment | null {
        let date;
        if (value instanceof Date) date = jalaliMoment(value);
